...
[`lc50()`](https://open-aims.github.io/toxstats/reference/lc50.md) walks
the Figure 6 chart of the acute manual. Each of the four columns of its
Table 20 was constructed to exercise one of the four methods, and each
...
implements the one ToxCalc feature that is not an EPA method. Neither
manual gives a worked example or a table of critical values for it, so
the values are simulated and the implementation is validated against
mathematical identities instead. It is labelled as an extension
throughout, and neither flowchart will ever select it.
